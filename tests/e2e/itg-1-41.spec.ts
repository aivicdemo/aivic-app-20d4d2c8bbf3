...
test.describe("工数記録データ抽出画面", () => {
  ...
  test("SCEN-303: 異常値除外条件でデータ抽出される", async ({ page }) => {
    ...
    await page.click('[data-testid="extract-button"]');
    await page.waitForSelector('[data-testid="data-count"]');
    
    const dataCount = await page.textContent('[data-testid="data-count"]');
    expect(dataCount).toContain('件');
  });
  ...
  test("SCEN-305: 抽出条件プレビューが正しく表示される", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="preview-section"]');
    
    const previewContent = await page.textContent('[data-testid="preview-content"]');
    expect(previewContent).toContain('2024-01-01');
    expect(previewContent).toContain('2024-01-31');
  });
  ...
  test("SCEN-308: 条件クリアで全項目がリセットされる", async ({ page }) => {
    ...
    expect(await page.inputValue('[data-testid="start-date"]')).toBe('');
    expect(await page.inputValue('[data-testid="end-date"]')).toBe('');
    expect(await page.inputValue('[data-testid="work-location"]')).toBe('');
  });
  ...
  test("SCEN-311: 作業員未選択で抽出実行時にエラー表示", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="error-message"]')).toBeVisible();
    const errorText = await page.textContent('[data-testid="error-message"]');
    expect(errorText).toContain('作業員');
  });
  ...
  test("SCEN-312: 出力形式未選択で抽出実行時にエラー表示", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="error-message"]')).toBeVisible();
    const errorText = await page.textContent('[data-testid="error-message"]');
    expect(errorText).toContain('出力形式');
  });
  ...
  test("SCEN-313: 大量データ抽出時のタイムアウトエラー", async ({ page }) => {
    ...
    const errorText = await page.textContent('[data-testid="error-message"]');
    expect(errorText).toMatch(/(タイムアウト|時間|処理)/);
  });
  ...
  test("SCEN-314: 当日日付での期間指定", async ({ page }) => {
    ...
    const dataCount = await page.textContent('[data-testid="data-count"]');
    expect(dataCount).toMatch(/\d+件|データなし/);
  });
  ...
  test("SCEN-315: 最大期間範囲での抽出", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="loading"], [data-testid="data-count"], [data-testid="error-message"]')).toBeVisible({ timeout: 30000 });
  });
  ...
  test("SCEN-317: 抽出結果0件の場合の表示", async ({ page }) => {
    ...
    const dataCount = await page.textContent('[data-testid="data-count"]');
    expect(dataCount).toMatch(/0件|データなし|見つかりませんでした/);
  });
  ...
  test("SCEN-318: 最大件数上限での抽出", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="data-count"], [data-testid="error-message"]')).toBeVisible({ timeout: 15000 });
    
    const hasWarning = await page.locator('text=上限, text=最大').count() > 0;
    const dataCount = await page.textContent('[data-testid="data-count"]');
    
    expect(hasWarning || dataCount?.includes('件')).toBeTruthy();
  });
});