...
test.describe("作業履歴一覧画面", () => {
  ...
  test('SCEN-033: 検索ボタンで結果更新される', async ({ page }) => {
    // SCEN-033
    await page.fill('input[name="startDate"]', '2024-01-01');
    await page.fill('input[name="worker"]', 'テスト太郎');
    await page.fill('input[name="content"]', '配線');
  ...
  test('SCEN-035: 作業履歴一覧が正しく表示される', async ({ page }) => {
    ...
    await expect(page.locator('.date-column')).toBeVisible();
  ...
  test('SCEN-038: 1年以上の期間選択でエラー表示', async ({ page }) => {
    ...
    await page.click('button:has-text("適用")');
    await expect(page.locator('.error-message')).toBeVisible();
    await expect(page.locator('.error-message')).toContainText('1年以上の期間');
  });

  test('SCEN-039: 検索結果0件時の表示', async ({ page }) => {
    // SCEN-039
    await page.fill('input[name="startDate"]', '2099-01-01');
    await page.fill('input[name="worker"]', '存在しない作業員');
    await page.click('button:has-text("検索")');
    await expect(page.locator('.no-results-message')).toBeVisible();
    await expect(page.locator('.no-results-message')).toContainText('該当する作業履歴が見つかりません');
  ...
  test('SCEN-040: 大量データ表示時のパフォーマンス', async ({ page }) => {
    ...
    await page.locator('.work-history-list').scrollIntoView({ block: 'end' });
