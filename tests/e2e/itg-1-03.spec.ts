...
test.describe("作業履歴一覧画面", () => {
  ...
  test("SCEN-029: 日付範囲選択で作業履歴を絞り込める", async ({ page }) => {
    // SCEN-029
    await page.click('#input-start-date');
    await page.fill('#input-start-date', '2024-01-01');
    await page.click('#input-end-date');
  ...
  test("SCEN-030: 作業項目ドロップダウンで絞り込める", async ({ page }) => {
    // SCEN-030
    await page.locator('#select-work-item').click();
    await page.locator('#select-work-item').selectOption('配線工事');
    await page.waitForTimeout(1000);
    await expect(page.locator('[data-testid="work-history-table"]')).toBeVisible();
    
    await page.locator('#select-work-item').selectOption('点検作業');
    await page.waitForTimeout(1000);
    await expect(page.locator('[data-testid="work-history-table"]')).toBeVisible();
    
    await page.locator('#select-work-item').selectOption('');
  });

  test("SCEN-031: 作業状況フィルターで絞り込める", async ({ page }) => {
    // SCEN-031
    await page.locator('#select-status').click();
    await page.locator('#select-status').selectOption('進行中');
    await page.click('#btn-search');
    await expect(page.locator('[data-testid="work-history-table"]')).toBeVisible();
    
    await page.locator('#select-status').selectOption('完了');
    await page.click('#btn-search');
    await expect(page.locator('[data-testid="work-history-table"]')).toBeVisible();
    
    await page.locator('#select-status').selectOption('');
    await expect(page.locator('[data-testid="work-history-table"]')).toBeVisible();
  });
  ...
  test("SCEN-032: 複数フィルター組み合わせで絞り込める", async ({ page }) => {
    ...
    await page.fill('#input-end-date', '2024-01-31');
    await page.locator('#select-work-item').selectOption('点検作業');
    await page.locator('#select-worker').selectOption('田中太郎');
    await page.click('#btn-search');
    await expect(page.locator('[data-testid="work-history-table"]')).toBeVisible();
  });
  ...
  test("SCEN-039: 検索結果0件時の表示", async ({ page }) => {
    // SCEN-039
    await page.fill('#input-start-date', '1990-01-01');
    await page.fill('#input-end-date', '1990-01-02');
    await page.locator('#select-worker').selectOption('存在しない作業員');
    await page.click('#btn-search');
