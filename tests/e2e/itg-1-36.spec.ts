...
test.describe("工数入力画面", () => {
  ...
  test("SCEN-210: カレンダーから作業日付を選択できる", async ({ page }) => {
    ...
    await expect(page.locator('.calendar, [type="date"]')).toBeVisible();
    await page.fill('[data-testid="work-date"]', '2024-01-15');
    
    await expect(page.locator('[data-testid="work-date"]')).toHaveValue('2024-01-15');
  });
  ...
  test("SCEN-212: 開始終了時刻入力で実働時間が自動計算される", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="actual-work-time"]')).toContainText('8:30');
  });
  ...
  test("SCEN-213: 休憩時間入力で実働時間が正しく調整される", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="actual-work-time"]')).toContainText('7:00');
  ...
  test("SCEN-214: 中断記録を追加できる", async ({ page }) => {
    ...
    await page.selectOption('[data-testid="work-item"]', { index: 1 });
    await page.fill('[data-testid="start-time"]', '09:00');
    await page.selectOption('[data-testid="progress-status"]', 'suspended');
    await page.selectOption('[data-testid="interrupt-reason"]', { index: 1 });
    await page.fill('[data-testid="interrupt-start"]', '10:30');
  ...
  test("SCEN-215: 複数の中断記録を登録できる", async ({ page }) => {
    ...
    await page.selectOption('[data-testid="interrupt-reason"]', 'material-check');
    ...
    await page.fill('[data-testid="interrupt-start"]', '15:00');
    await page.click('[data-testid="add-interrupt-button"]');
  ...
  test("SCEN-222: 24時間をまたぐ作業時間入力", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="actual-work-time"]')).toContainText('3:00');
  });
  ...
  test("SCEN-223: 休憩時間0分での登録", async ({ page }) => {
    ...
    await page.selectOption('[data-testid="work-item"]', { index: 1 });
    await page.fill('[data-testid="work-details"]', '作業内容');
    await page.click('[data-testid="save-button"]');
    
    await expect(page.locator('[data-testid="actual-work-time"]')).toContainText('8:00');
  });
