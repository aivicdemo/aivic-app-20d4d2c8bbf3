...
test.describe("修正入力画面", () => {
  ...
  test("SCEN-249: 修正対象記録選択して正常修正できる", async ({ page }) => {
    ...
    await page.fill('[data-testid="correction-reason"]', '作業内容の修正');
  ...
  test("SCEN-250: 作業日付を正常に変更できる", async ({ page }) => {
    // SCEN-250
    await page.selectOption('[data-testid="target-record"]', { index: 0 });
    await page.fill('[data-testid="work-date"]', '2024-01-20');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('[data-testid="work-date"]')).toHaveValue('2024-01-20');
  });
  ...
  test("SCEN-251: 作業開始終了時刻を正常に修正できる", async ({ page }) => {
    ...
    await page.fill('[data-testid="end-time"]', '16:30');
    await page.fill('[data-testid="correction-reason"]', '作業時刻修正');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('修正完了');
  });
  ...
  test("SCEN-258: 作業日付に無効な日付でエラー", async ({ page }) => {
    ...
    await page.fill('[data-testid="correction-reason"]', '修正理由');
    await page.click('[data-testid="save-button"]');
  ...
  test("SCEN-262: 異常値警告が正しく表示される", async ({ page }) => {
    // SCEN-262
    await page.selectOption('[data-testid="target-record"]', { index: 0 });
    await page.fill('[data-testid="start-time"]', '-5');
    await page.fill('[data-testid="end-time"]', '25:00');
    await page.fill('[data-testid="work-date"]', '2099-12-31');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#anomaly-warnings')).toBeVisible();
    await expect(page.locator('#error-messages')).toContainText('異常値');
  });
  ...
  test("SCEN-264: 時刻境界値00:00と23:59で正常動作", async ({ page }) => {
    ...
    await page.fill('[data-testid="correction-reason"]', '境界値時刻テスト');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('保存完了');
  });
  ...
  test("SCEN-265: 修正理由文字数上限での動作", async ({ page }) => {
    ...
    await page.type('[data-testid="correction-reason"]', 'x');
    const value = await page.inputValue('[data-testid="correction-reason"]');
