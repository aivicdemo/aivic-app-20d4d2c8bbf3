...
test.describe("修正入力画面", () => {
  ...
  test("SCEN-249: 修正対象記録選択して正常修正できる", async ({ page }) => {
    // SCEN-249
    await page.selectOption('[data-testid="target-record"]', { index: 1 });
    ...
    await page.fill('[data-testid="correction-reason"]', '作業内容修正のため');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('修正が完了しました');
  });
  ...
  test("SCEN-252: 作業項目を変更して修正できる", async ({ page }) => {
    // SCEN-252
    await page.selectOption('[data-testid="target-record"]', { index: 1 });
    await page.selectOption('[data-testid="work-item"]', { index: 2 });
  ...
  test("SCEN-253: 中断時刻と理由を修正できる", async ({ page }) => {
    // SCEN-253
    await page.selectOption('[data-testid="target-record"]', { index: 1 });
    await page.fill('[data-testid="break-time"]', '12:30');
    await page.selectOption('[data-testid="break-reason"]', { index: 1 });
  ...
  test("SCEN-254: 修正理由を入力して保存できる", async ({ page }) => {
    ...
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('保存が完了しました');
  });
  ...
  test("SCEN-256: 実作業時間が自動計算される", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="actual-work-time"]')).toContainText('7時間30分');
  });

  test("SCEN-257: 修正対象記録未選択でエラー", async ({ page }) => {
    // SCEN-257
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('修正対象の記録が選択されていません');
  });
  ...
  test("SCEN-258: 作業日付に無効な日付でエラー", async ({ page }) => {
    ...
    await page.fill('[data-testid="correction-reason"]', '日付修正');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('有効な日付を入力してください');
  });
  ...
  test("SCEN-259: 開始時刻が終了時刻より後でエラー", async ({ page }) => {
    ...
    await page.fill('[data-testid="correction-reason"]', '時刻修正');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('開始時刻は終了時刻より前の時刻を入力してください');
  });
  ...
  test("SCEN-260: 中断時刻が作業時間外でエラー", async ({ page }) => {
    ...
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('中断時刻が作業時間外です');
  });
  ...
  test("SCEN-261: 修正理由未入力でエラー", async ({ page }) => {
    ...
    await page.fill('[data-testid="end-time"]', '17:00');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('修正理由を入力してください');
  });
  ...
  test("SCEN-262: 異常値警告が正しく表示される", async ({ page }) => {
    ...
    await page.fill('[data-testid="work-date"]', '2025-12-31');
    await page.fill('[data-testid="correction-reason"]', '異常値テスト');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#anomaly-warnings')).toBeVisible();
    await expect(page.locator('#error-messages')).toContainText('異常な値が含まれています');
  });
  ...
  test("SCEN-263: 作業日付境界値で正常動作", async ({ page }) => {
    ...
    await page.fill('[data-testid="correction-reason"]', '境界値テスト1');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).not.toContainText('エラー');
    
    await page.fill('[data-testid="work-date"]', '2099-12-31');
    await page.fill('[data-testid="correction-reason"]', '境界値テスト2');
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).not.toContainText('エラー');
  });
  ...
  test("SCEN-264: 時刻境界値00:00と23:59で正常動作", async ({ page }) => {
    ...
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('#error-messages')).toContainText('修正が完了しました');
    await expect(page.locator('[data-testid="start-time"]')).toHaveValue('00:00');
    await expect(page.locator('[data-testid="end-time"]')).toHaveValue('23:59');
  });
  ...
  test("SCEN-265: 修正理由文字数上限での動作", async ({ page }) => {
    ...
    await page.fill('[data-testid="correction-reason"]', maxText + 'x');
    const actualValue = await page.locator('[data-testid="correction-reason"]').inputValue();
