...
test.describe("工数入力画面", () => {
  ...
  test('SCEN-209: 全項目入力で工数記録が正常登録される', async ({ page }) => {
    ...
    await page.fill('[data-testid="work-details"]', '詳細な作業内容');
  ...
  test('SCEN-210: カレンダーから作業日付を選択できる', async ({ page }) => {
    ...
    const dateValue = await page.locator('[data-testid="work-date"]').inputValue();
  ...
  test('SCEN-211: 作業項目をドロップダウンから選択できる', async ({ page }) => {
    ...
    await page.selectOption('[data-testid="work-item"]', { index: 1 });
    const selectedValue = await page.locator('[data-testid="work-item"]').inputValue();
  ...
  test('SCEN-212: 開始終了時刻入力で実働時間が自動計算される', async ({ page }) => {
    await page.fill('[data-testid="start-time"]', '09:00');
    await page.fill('[data-testid="end-time"]', '17:30');
    await page.click('[data-testid="work-details"]');
    const actualWorkTime = await page.locator('[data-testid="actual-work-time"]').inputValue();
    expect(actualWorkTime).toBe('8:30');
  });
  ...
  test('SCEN-213: 休憩時間入力で実働時間が正しく調整される', async ({ page }) => {
    ...
    await page.click('[data-testid="work-details"]');
    const actualWorkTime = await page.locator('[data-testid="actual-work-time"]').inputValue();
    expect(actualWorkTime).toBe('7:00');
    await page.click('[data-testid="save-button"]');
  });
  ...
  test('SCEN-214: 中断記録を追加できる', async ({ page }) => {
    await page.selectOption('[data-testid="work-item"]', { index: 1 });
    await page.fill('[data-testid="start-time"]', '09:00');
    await page.selectOption('[data-testid="progress-status"]', '中断');
    await page.selectOption('[data-testid="interrupt-reason"]', '資材確認');
  ...
  test('SCEN-215: 複数の中断記録を登録できる', async ({ page }) => {
    ...
    await page.click('[data-testid="interrupt-button"]');
    await page.selectOption('[data-testid="interrupt-reason"]', '打ち合わせ');
    await page.fill('[data-testid="interrupt-start"]', '14:00');
    await page.click('[data-testid="add-interrupt-button"]');
    await page.click('button:has-text("作業再開")');
    await page.click('[data-testid="interrupt-button"]');
    await page.selectOption('[data-testid="interrupt-reason"]', '休憩');
    await page.fill('[data-testid="interrupt-start"]', '16:00');
    await page.click('[data-testid="add-interrupt-button"]');
    await expect(page.locator('[data-testid="interrupt-list"]')).toContainText('資材確認');
    await expect(page.locator('[data-testid="interrupt-list"]')).toContainText('打ち合わせ');
    await expect(page.locator('[data-testid="interrupt-list"]')).toContainText('休憩');
  });
  ...
  test('SCEN-218: 未来日付選択でエラー', async ({ page }) => {
    ...
    await page.fill('[data-testid="work-date"]', tomorrowStr);
    await page.selectOption('[data-testid="work-item"]', { index: 1 });
    await page.fill('[data-testid="start-time"]', '09:00');
    await page.fill('[data-testid="end-time"]', '17:00');
    await page.click('[data-testid="save-button"]');
  ...
  test('SCEN-219: 時刻フォーマット不正でエラー', async ({ page }) => {
    ...
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('[data-testid="error-message"]')).toBeVisible();
    await expect(page.locator('[data-testid="error-message"]')).toContainText('フォーマット');
  });
  ...
  test('SCEN-222: 24時間をまたぐ作業時間入力', async ({ page }) => {
    ...
    await page.click('[data-testid="work-details"]');
    const actualWorkTime = await page.locator('[data-testid="actual-work-time"]').inputValue();
    expect(actualWorkTime).toBe('3:00');
    await page.click('[data-testid="save-button"]');
  });
  ...
  test('SCEN-223: 休憩時間0分での登録', async ({ page }) => {
    ...
    await page.click('[data-testid="save-button"]');
    const actualWorkTime = await page.locator('[data-testid="actual-work-time"]').inputValue();
    expect(actualWorkTime).toBe('8:00');
  });
  ...
  test('SCEN-225: 作業内容詳細の最大文字数入力', async ({ page }) => {
    ...
    await page.fill('[data-testid="end-time"]', '17:00');
    await page.selectOption('[data-testid="work-item"]', { index: 1 });
    const maxLengthText = 'あ'.repeat(1000);
    await page.fill('[data-testid="work-details"]', maxLengthText);
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('.success-message')).toBeVisible();
  });
  ...
  test('SCEN-226: 中断時間の合計が実働時間と一致する場合', async ({ page }) => {
    ...
    await page.click('[data-testid="save-button"]');
    await expect(page.locator('[data-testid="error-message"]')).toBeVisible();
    await expect(page.locator('[data-testid="error-message"]')).toContainText('中断時間の合計');
  });
});