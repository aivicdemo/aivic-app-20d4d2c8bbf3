...
test.describe("工数記録画面", () => {
  ...
  test("SCEN-014: 作業開始から終了まで正常記録", async ({ page }) => {
    ...
    await page.fill('#work-memo', '設備点検作業');
    await page.waitForTimeout(3000);
    await page.click('#end-btn');
    await page.click('#save-btn');
    await expect(page.locator('#today-work-history')).toBeVisible();
  });
  ...
  test("SCEN-015: 作業中断・再開が正常動作", async ({ page }) => {
    ...
    await page.fill('#pause-reason', '材料補充');
  ...
  test("SCEN-016: 作業メモ入力して保存", async ({ page }) => {
    ...
    await page.click('#save-btn');
    await expect(page.locator('text=設備点検時に異常音を確認。次回要詳細調査')).toBeVisible();
  });
  ...
  test("SCEN-025: 24時間連続作業記録", async ({ page }) => {
    ...
    await page.click('#start-btn');
    await page.clock.setSystemTime(new Date('2024-01-01T23:59:00'));
    await page.click('#end-btn');
  ...
  test("SCEN-026: 日付跨ぎ作業の記録", async ({ page }) => {
    // SCEN-026
    await page.selectOption('#work-item-select', { index: 1 });
    await page.clock.install({ time: new Date('2024-01-01T23:30:00') });
    await page.click('#start-btn');
    await page.clock.setSystemTime(new Date('2024-01-02T01:30:00'));
    await page.click('#end-btn');
  ...
  test("SCEN-028: 経過時間の正確な表示更新", async ({ page }) => {
    ...
    await page.waitForTimeout(1000);
    await expect(page.locator('#elapsed-time')).toContainText('00:00:01');
    await page.evaluate(() => document.hidden = true);
    await page.waitForTimeout(1000);
    await page.evaluate(() => document.hidden = false);
    await expect(page.locator('#elapsed-time')).toContainText('00:00:02');
    await page.click('#end-btn');
  });
});